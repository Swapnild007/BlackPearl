...
import { BookOpen, BrainCircuit, ChevronRight, CircleUserRound, FlaskConical, GraduationCap, Menu, Moon, Search, Sparkles, Sun, X } from "lucide-react";
...
function Shell({ children, dark, toggle }: {children: React.ReactNode; dark:boolean; toggle:()=>void}) {
  ...
  const links=[["Learn","/learn"],["Curriculum","/curriculum"],["Projects","/projects"],["Academic","/academic"]];
...
function Home() {
 ...
 <section className="signal-grid">
   {[[BookOpen,"6","Core modules"],[BrainCircuit,"12","Project directions"],[GraduationCap,"4","Academic stages"]].map(([Icon,num,label])=><div className="signal" key={label as string}><Icon size={19}/><strong>{num}</strong><span>{label}</span></div>)}
 </section>
 <section className="method">
   <div><span className="eyebrow">THE BLACKPEARL METHOD</span><h2>Not a video playlist.<br/>A study system.</h2></div>
...
function Learn() {
 const [q,setQ]=useState(""); const filtered=useMemo(()=>modules.filter(m=>(m.title+" "+m.topics.join(" ")).toLowerCase().includes(q.toLowerCase())),[q]);
...
function ModuleRoute(){const location=useLocation(); return <Module id={location.pathname.split("/").pop()||"01"}/>}
