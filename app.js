...
const seed=typeof BP_TOPIC_SEEDS!=="undefined"?BP_TOPIC_SEEDS[title]:null;const domain=(typeof BP_DOMAINS!=="undefined"?(BP_DOMAINS.find(d=>d[1].includes(title))||[])[0]:"Curriculum");const l=BP_LESSONS[title]||{domain:domain||"Curriculum",objective:"Understand "+title+" as a mechanism that can be predicted, derived, implemented and transferred.",mental:seed?.[0]||("Build a first-principles mental model of "+title+"."),predict:seed?.[1]||("What qualitative behavior should you expect from "+title+" before calculating or implementing it?"),options:[seed?.[1]||"The expected mechanism occurs","Nothing changes","The opposite mechanism occurs","The operation is undefined"],answer:0,derive:seed?.[2]||("Write the defining relationship for "+title+" and explain each symbol."),transfer:seed?.[4]||("Connect "+title+" to an unfamiliar ML system using a causal chain.")};
