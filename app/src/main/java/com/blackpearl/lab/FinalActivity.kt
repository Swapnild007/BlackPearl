...
class FinalActivity : Activity() {
    ...
    private val border = Color.rgb(42, 53, 65)
    ...
    private fun home() {
        ...
        body.addView(module("03", "WEB", "HTTP INSPECTION", "Headers and status for localhost only") { web() })
        body.addView(section("ANALYSIS"))
        body.addView(module("04", "ANDROID", "SECURITY POSTURE", "Package, build and debug-state inspection") { android() })
        body.addView(module("05", "CRYPTO", "LOCAL UTILITIES", "SHA-256, SHA-512 and Base64") { crypto() })
    ...
    private fun session(): View {
        val box = LinearLayout(this)
        box.orientation = LinearLayout.VERTICAL
        box.setPadding(dp(12), dp(10), dp(12), dp(10))
        box.background = border(panel)
    ...
    private fun web() {
        val b = base(); b.addView(label("HTTP INSPECTOR", white, 18f)); b.addView(label("HEAD request. Localhost only.", muted, 11f))
        val input = input("http://127.0.0.1:8080/"); val out = output(); b.addView(input); b.addView(out)
        b.addView(action("INSPECT") {
            try {
                val url = URL(input.text.toString().trim())
                if (!allowed(url.host)) { out.text = "BLOCKED\nLOCALHOST ONLY" } else Thread {
                    try {
                        val c = url.openConnection() as HttpURLConnection
                        c.requestMethod = "HEAD"; c.connectTimeout = 2500; c.readTimeout = 2500; c.connect()
                        val result = StringBuilder("HTTP ${c.responseCode} ${c.responseMessage}\n\n")
                        for ((key, values) in c.headerFields) if (key != null) result.append(key).append(": ").append(values.joinToString(", ")).append('\n')
                        c.disconnect()
                        runOnUiThread { out.text = result.toString(); log("WEB", "Inspected localhost") }
                    } catch (e: Exception) { runOnUiThread { out.text = "ERROR\n${e.message ?: "Request failed"}" } }
                }.start()
            } catch (_: Exception) { out.text = "INVALID URL" }
        })
        screen("WEB", b)
    }
    ...
    private fun findings() {
        val b = base(); b.addView(label("FINDINGS / CASE TRACKER", white, 18f)); b.addView(label("Local evidence notes. Nothing is uploaded.", muted, 11f))
    ...
    private fun input(hintText: String): EditText = EditText(this).apply {
        hint = hintText; setTextColor(white); setHintTextColor(muted); textSize = 13f; setSingleLine(false); setPadding(dp(10), dp(9), dp(10), dp(9)); background = border(panel2)
    }

    private fun output(): TextView = TextView(this).apply {
        setTextColor(white); textSize = 11f; typeface = Typeface.MONOSPACE; setPadding(dp(10), dp(10), dp(10), dp(10)); background = border(panel); minHeight = dp(84)
    }

    private fun code(value: String): TextView = TextView(this).apply {
        text = value; setTextColor(Color.rgb(175, 225, 220)); textSize = 11f; typeface = Typeface.MONOSPACE; setPadding(dp(10), dp(10), dp(10), dp(10)); background = border(panel)
    }
    ...
    private fun consoleView(): TextView = TextView(this).apply {
        setTextColor(Color.rgb(160, 225, 215)); textSize = 10f; typeface = Typeface.MONOSPACE; setPadding(dp(10), dp(10), dp(10), dp(10)); background = border(Color.rgb(8, 12, 16)); minHeight = dp(140)
    }
    ...
    private fun border(color: Int): android.graphics.drawable.GradientDrawable = android.graphics.drawable.GradientDrawable().apply { setColor(color); setStroke(dp(1), border); cornerRadius = dp(2).toFloat() }
